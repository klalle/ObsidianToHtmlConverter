...
if(len(sys.argv) != 2 and len(sys.argv) != 3 and len(sys.argv) != 4 and len(sys.argv) != 5):
    print(
        "Wrong number of arguments!\nUsage: python3 exportMdFileToHtml.py <filename.md> <destination_path(optional) => path to export to, default is ~/export_filename> <[y/n](optional) y=default => creates a html-export in export vault> <[y/n](optional) y=default => download extrernal images locally>")
    quit()
...
exportToHtml = True
downloadImages = True
exportPath = "~/export_" + fileToFind.split('/')[-1].replace(".md", "")
if len(sys.argv) >= 3:
    exportPath = str(sys.argv[2])
    if len(sys.argv) == 4 and str(sys.argv[3]).upper() == "N":
        print("Exporting: " + str(mainFileToExport) + " to vault")
        exportToHtml = False
    if len(sys.argv) == 5 and str(sys.argv[4]).upper() == "N":
        downloadImages = False
else:
    print("Exporting: " + str(mainFileToExport) +
          " + creates a html-copy in vault")


if(mainFileToExport == ""):
    print(
        "File not found!\nRun this script from the root of obsidian vault\nUsage: python3 exportMdFileToHtml.py <filename.md> <destination_path(optional) => path to export to, default is ~/export_filename> <[y/n](optional) y=default => creates a html-export in export vault>")
    quit()

exportDir = os.path.expanduser(exportPath)
print("Path to export vault: " + str(exportDir) + "\n")

if os.path.exists(exportDir) and os.path.isdir(exportDir):
    if os.path.exists(exportDir+"/.git") and os.path.isdir(exportDir+"/.git"):
        with tempfile.TemporaryDirectory() as tmpdirname:
            shutil.copytree(exportDir+"/.git", tmpdirname+"/.git")
            shutil.rmtree(exportDir)
            destFile = os.path.join(exportDir, mainFileToExport)
            Path(os.path.dirname(destFile)).mkdir(parents=True, exist_ok=True)
            shutil.copytree(tmpdirname+"/.git", exportDir+"/.git")
    else:
        shutil.rmtree(exportDir)
        destFile = os.path.join(exportDir, mainFileToExport)
        Path(os.path.dirname(destFile)).mkdir(parents=True, exist_ok=True)
...
def findMdFile(line, currentFile):
    ...
    for (file) in re.findall(pattern, line):
        ...
        if(len(file.split("#")) > 1):
            ancor = "#" + \
                file.split("#")[1].replace(" ", "_").replace(
                    "(", "").replace(")", "")
        newFile = copyFileToExport(
            fileOnly + '.md', currentFile, traverse=True)
        if(exportToHtml):
            if(newFile and len(newFile) > 0):
                line = line.replace('[[' + file + ']]', '<a href="./' + newFile + ".html" + ancor +
                                    '">' + newFile.replace("\\", "/").split("/")[-1].replace(".md", "") + ancor + '</a>')
            else:  # self ref
                line = line.replace('[[' + file + ']]', '<a href="./' + fileOnly + ".md.html" + ancor +
                                    '">' + fileOnly.replace("\\", "/").split("/")[-1].replace(".md", "") + ancor + '</a>')
    return line
...
def findImages(line, currentFile):
    ...
    for (asset) in re.findall(pattern, line):
        ...
        if(exportToHtml):
            ...
            if('|' in asset):
                style = style + 'width:' + \
                    asset.split('|')[1] + 'px; border-radius: 3px;'
            line = line.replace("![[" + asset + "]]", '<img src="./' + img + '" alt="' +
                                img.replace("\\", "/").split("/")[-1] + '" style="' + style + '" >')
    ...
    for size, imglink in re.findall(pattern, line):
        ...
        if(exportToHtml):
            if("http" not in imglink):
                ...
                imglink = str(copyFileToExport(
                    unquote(imglink.replace("\\", "/").split("/")[-1]), currentFile))

                style = 'border-radius: 4px;"'
                if('|' in imglink):
                    style = style + 'width:' + \
                        imglink.split('|')[1] + 'px; border-radius: 3px;'
                line = line.replace("![" + size + "](" + originallink + ")", '<img src="./' + imglink +
                                    '" alt="' + imglink.replace("\\", "/").split("/")[-1] + '" style="' + style + '" >')
            elif downloadImages:
                imgname = 'utl_download_' + \
                    str(randint(0, 10000)) + imglink.split("/")[-1]
                destFile = os.path.join(
                    exportDir, "downloaded_images", imgname)
                with urllib.request.urlopen(imglink) as responese:
                    with open(destFile, 'wb') as fdest:
                        copyfileobj(responese, fdest)

                style = 'border-radius: 4px;"'
                line = line.replace("![" + size + "](" + imglink + ")",
...
def findLinkInText(line):
    ...
    for (link) in re.findall(pattern, line):
        line = line.replace(link, '<a href="' + link.strip() +
...
def findCodeBlock(line, InCodeBlock):
    if("```" in line):
        if(not InCodeBlock):
            line = '<pre class="codeblock"><code style="tab-size: 4;" class="' + \
...
def findListItems(line):
    ...
    for (tab, text) in re.findall(pattern, line):
        line = '<ul style="margin-left:' + \
            str(len(tab) * 20) + 'px;"><li>' + text.strip() + '</li></ul>\n'
    return line
...
def findHeadings(line):
    ...
    for (tab, heading, link, text, aftertext) in re.findall(linkHeading, line):
        line = '<h' + str(len(heading)) + ' style="margin-left:' + str(len(tab) * 20) + 'px;" id="' + (
            (text + aftertext).strip().replace(" ",
                                               "_").replace("(", "").replace(")", "").replace("#", "_") + '">'
            + link + aftertext + '</h' + str(len(heading)) + '>\n')

    for (tab, heading, text) in re.findall(pattern, line):
        line = '<h' + str(len(heading)) + ' style="margin-left:' + str(len(tab) * 20) + 'px;" id="' + text.strip(
        ).replace(" ", "_").replace("(", "").replace(")", "") + '">' + text + '</h' + str(len(heading)) + '>\n'
    return line
...
def findInlineCodeBlocks(line):
    ...
    for (text) in re.findall(pattern, line):
        line = line.replace(
            '`' + text + '`', '<code class="inlineCoed">' + html.escape(text) + '</code>')
    return line
...
def readFilesRecursive(path):
    ...
    if(exportToHtml):
        with open(os.path.join(exportDir, str(path) + ".html"), 'w', encoding='utf-8') as outputfile:
            ...
            outputfile.write('\t<iframe src="' + str(findRelPath(".", path))[
                             :-1] + 'treeview.html" width="340px" frameBorder="0" height="900px"></iframe>\n')
            outputfile.write("</div>\n")
...
if(exportToHtml):
    ...
    with open(os.path.join(exportDir, "treeview.html"), 'w') as outputfile:
        ...
        for f in str(filesAllreadyCopied[0]).replace("\\", "/").split("/"):
            if('.md' in f):
                outputfile.write('<li>' + '<a href="./' + str(
                    filesAllreadyCopied[0]) + ".html" + '">' + str(f).replace(".md", "") + '</a>' + '</li>\n')
            else:
                outputfile.write('<li class="folderClass">' + str(f) + "</li>")
                outputfile.write("<ul>")
        lastFilePath = str(filesAllreadyCopied[0]).replace(
            "\\", "/").split("/")
        first = True
            ...
            if(not first):
                for currFolder in str(currFile).replace("\\", "/").split("/"):
                    if(len(lastFilePath) > 0 and currFolder == lastFilePath[0]):
                        ...
                        for addedSublist in lastFilePath[:-1]:
                            outputfile.write("</ul>\n")
                        lastFilePath = ""
                        if('.md' in currFolder):
                            outputfile.write('<li>' + '<a href="./' + str(currFile) + ".html" + '">' + str(
                                currFolder).replace(".md", "") + '</a>' + '</li>\n')
                        else:
                            outputfile.write(
